...
class UnivariateZDecayHawkesMLE(BaseUnivariateHawkesMLE):
    """
    Hawkes avec decay modulé par marks_k.

    lambda(t)=mu+sum_{t_k<t} alpha*beta*exp(-beta*(t-t_k)/marks_k)

    Les marks_k sont observés et doivent être strictement positifs.
    Le calcul exact est en O(n^2) si les marks_k sont arbitraires.
    """
    ...
    @staticmethod
    def _kernel_integral_and_grad(times,marks,T,beta):
        if len(times)==0:
            return 0.0,0.0

        rem=T-times
        e=np.exp(-beta*rem/marks)

        S=np.sum(marks*(1.0-e))
        S_beta=np.sum(rem*e)

        return float(S),float(S_beta)

    def _nll_grad_one(self,theta,times,marks,T):
        p=self._unpack(theta)
        mu,alpha,beta=p["mu"],p["alpha"],p["beta"]
        ...
        for k in range(1,n):
            dt=times[k]-times[:k]
            past_marks=marks[:k]
            e=np.exp(-beta*dt/past_marks)

            A[k]=np.sum(e)
            B[k]=np.sum((dt/past_marks)*e)
        ...
        inv_lam=1.0/lam
        S,S_beta=self._kernel_integral_and_grad(times,marks,T,beta)

        ll=np.sum(np.log(lam))-mu*T-alpha*S
        grad_mu=np.sum(inv_lam)-T
        grad_alpha=np.sum(beta*A*inv_lam)-S
        grad_beta=np.sum(alpha*(A-beta*B)*inv_lam)-alpha*S_beta
    ...
    def _post_fit(self,payload):
        ...
        all_marks=np.concatenate(payload) if sum(len(x) for x in payload)>0 else np.array([])

        self.mean_mark_=float(np.mean(all_marks)) if len(all_marks)>0 else 1.0
        self.branching_ratio_empirical_=float(self.alpha_*self.mean_mark_)
    ...
    def intensity_at_events(self,events=None,marks=None):
        ...
        times,marks=self._prepare_one_realization(events,marks)
        ...
        for k in range(1,n):
            A[k]=np.sum(np.exp(-self.beta_*(times[k]-times[:k])/marks[:k]))

        return self.baseline_+self.alpha_*self.beta_*A
    ...
    def _kernel_cumulative_at(self,t,times,payload):
        ...
        past=times[mask]
        past_marks=marks[mask]

        return float(
            np.sum(
                past_marks*(1.0-np.exp(-self.beta_*(t-past)/past_marks))
            )
        )
    ...
    def _kernel_intensity_at(self,t,times,payload):
        ...
        return float(
            np.sum(
                self.beta_*np.exp(-self.beta_*(t-past)/past_marks)
            )
        )